...
def do_query_with_image_description(image_description: str, context: str, base64_image: str):
    # Load existing history
    history = []
    history_file = "history.json"
    if os.path.exists(history_file):
    ...
    system_message = """You are to help direct the user on how to do certain course materials. You are to provide information on how to complete the assignment.
                    ...
                    provide the context, the user does not know there was context provided, so do not mention any of the contexts directly.
                    Do not mention anything about a user query being provided by the way or repeat the user query!!
                    In this case, the user query is deciphered from an image. If the query seems to be not decipherable,
                    then literally ask for further context. Talk in the second person perspective to the user (example is: you)"""

    context = get_similar_with_query(image_description)
    user_message = """
    Deciphered from the image, the user seems to be querying about: {query}
    ...
    chain = prompt | chat
    message = chain.invoke({"query": image_description, "context": "\n".join(context)})
    
    # Add messages to history
    history.extend([
        {"type": "human", "message": base64_image},
        {"type": "assistant", "message": message.content}
    ])
